...
int _tmain(int argc, TCHAR* argv[])
{
    ...
    PdfTools_HttpClientHandler_SetClientCertificateAndKeyA(pHttpHandler, &sslCertDesc, &sslKeyDesc, "***insert password***");
    ...
    GOTO_CLEANUP_IF_NULL_PRINT_ERROR(
        pSession, _T("Failed to create GlobalSign DSS session. %s (ErrorCode: 0x%08x).\n"), szErrorBuff,
        PdfTools_GetLastError());

    // Create timestamp configuration
    pTimestamp = PdfToolsCryptoProvidersGlobalSignDss_Session_CreateTimestamp(pSession);
    GOTO_CLEANUP_IF_NULL_PRINT_ERROR(
        pTimestamp, _T("Failed to create timestamp configuration. %s (ErrorCode: 0x%08x).\n"), szErrorBuff,
        PdfTools_GetLastError());
